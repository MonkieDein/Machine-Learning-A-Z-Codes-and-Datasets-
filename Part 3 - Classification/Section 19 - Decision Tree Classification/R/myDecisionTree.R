# Decision Tree
#
# Decision Tree maximize the information entropy when added a condition (branch)
#
library(caTools) # For splitting test and train
# Set working directory
setwd("C:/GITHUB/Machine-Learning-A-Z-Codes-and-Datasets-/Part 3 - Classification/Section 19 - Decision Tree Classification/R")# Importing the dataset where string variable is treated as factor
...
# Building Classifier
library(rpart)
classifier = rpart(formula = Purchased ~ .,data = train_df)

# Making prediction on the test set
y_pred = predict(classifier,newdata = test_df[IV], type= 'class') 
...
set = train_df
X1 = seq(min(set[,1]),max(set[,1]),length.out = 150)
X2 = seq(min(set[,2]),max(set[,2]),length.out = 150)
